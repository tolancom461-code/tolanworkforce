...
## تنظيف البيانات الاختبارية لتسليم النظام
- [x] فحص جميع الجداول وتحديد البيانات الموجودة
- [x] حذف جميع البيانات الاختبارية بالترتيب الصحيح (مراعاة العلاقات)
- [x] التحقق من نظافة قاعدة البيانات - جميع الجداول فارغة ما عدا users (5)
- [x] حفظ checkpoint - 984c0a9e


## فحص وإصلاح مشكلة تداخل منتصف الليل - W01 يوم 5 فبراير
- [x] فحص بيانات البصمات في قاعدة البيانات - وجدت بصمة خروج 02:30 UTC يوم 5 تُنسب ليوم 5 بدل يوم 4
- [x] تحديد سبب استمرار المشكلة - getExpandedDateRange لا يوسع النطاق للخلف بشكل كافٍ + مشكلة timezone
- [x] إصلاح getExpandedDateRange - استخدام UTC صريحاً + توسيع النطاق 24 ساعة للخلف
- [x] ضبط timezone السيرفر على Asia/Riyadh (UTC+3) - موجود مسبقاً في index.ts
- [x] اختبار الإصلاح - 13 اختبار ناجح + اختبار يدوي للسيناريو
- [x] حفظ checkpoint - 926745d9

## إصلاح مشكلة دقائق العمل السالبة للورديات الليلية
- [x] إصلاح حساب دقائق العمل السالبة (-750) للورديات الليلية: حضور 3PM انصراف 2:30AM اليوم التالي يظهر -750 بدلاً من +690
- [x] تصحيح بيانات العامل W01 في قاعدة البيانات (يوم 5 و 10 فبراير)
- [x] إعادة حساب المالية للعامل W01
- [x] إضافة اختبارات vitest للورديات الليلية (6 اختبارات - جميعها نجحت)

## حذف بيانات الحضور والانصراف
- [x] حذف جميع سجلات attendance_events
- [x] حذف جميع سجلات worker_daily_finance
- [x] التحقق من نظافة الجداول

## تطوير سجل دفعات الراتب (Payroll Batches Log)
- [ ] حذف فلتر الحالة (Status) نهائياً من صفحة PayrollBatches
- [ ] إضافة خيار استعراض (View) في عمود الإجراءات للدفعات المعتمدة
- [ ] إضافة زر طباعة (Print) في صفحة تفاصيل الدفعة
- [ ] إضافة عمود "توقيع المستلم" فارغ في نهاية جدول التفاصيل (آخر عمود جهة اليسار)
- [ ] إضافة خيار حذف نهائي (Permanent Delete) للدفعات المعتمدة - يظهر فقط لـ Super Admin
- [ ] إضافة رسالة تأكيد قبل الحذف النهائي

## إصلاح صفحة سجل دفعات الرواتب (PayrollLog)
- [x] حذف فلتر الحالة (Status) نهائياً من صفحة سجل دفعات الرواتب
- [x] إضافة أزرار استعراض + طباعة + حذف نهائي في عمود الإجراءات
- [x] زر الحذف النهائي يظهر فقط لـ super_admin مع رسالة تأكيد
- [x] عمود توقيع المستلم في صفحة التفاصيل والطباعة

## ترتيب طباعة كشف الرواتب حسب المجموعات
- [x] ترتيب كشف الطباعة حسب المجموعات (اسم المجموعة ثم العمال تحتها)
- [x] إخفاء العمال الغائبين (غير موجودين في الدفعة)
- [x] إضافة 7 اختبارات vitest لتجميع العمال حسب المجموعات

## تنظيف البرنامج للتسليم النهائي
- [x] حذف الملفات المؤقتة (48 سكريبت .mjs + 49 ملف توثيق + صور + صفحات اختبارية + scripts/ + supabase/)
- [x] حذف console.log من db.ts, routers.ts, وجميع صفحات الفرونت إند
- [x] فحص TypeScript: 0 أخطاء
- [x] تشغيل 22 اختبار - جميعها نجحت
- [x] تنظيف قاعدة البيانات: حضور/انصراف فارغة، حذف مستخدمين اختباريين
- [x] فحص الواجهة النهائية: السيرفر يعمل، الواجهة سليمة
- [x] حفظ النسخة النهائية - eff4a91a

## إضافة زر طباعة في صفحة تفاصيل الدفعة
- [x] إضافة زر طباعة في صفحة /payroll/batches/:id (بدون عمود توقيع المستلم)
