...
## تحسينات واجهة المستخدم والصلاحيات
- [x] إخفاء عمود "كود الصلاحية" من صفحة إدارة الصلاحيات
- [x] إخفاء عمود "كود الصلاحية" من صفحة صلاحيات الأدوار
- [x] إخفاء عمود "كود الصلاحية" من صفحة صلاحيات المستخدمين
- [x] إزالة زر "إدارة الصلاحيات" القديم من صفحة المستخدمين
- [x] إضافة حقل "كلمة سر المستخدم" في form إضافة مستخدم
- [x] إضافة حقل "كلمة السر" في dialog تعديل المستخدم
- [x] تحديث backend API لدعم تعديل كلمة السر


## إصلاح تكرار الأدوار
- [x] فحص الأدوار الموجودة في قاعدة البيانات
- [x] حذف الأدوار المكررة والقديمة
- [x] الإبقاء على الأدوار الستة الثابتة فقط
- [x] اختبار صفحة الأدوار


## ربط الصلاحيات الافتراضية بالأدوار
- [x] تحديد الصلاحيات المناسبة لكل دور
- [x] ربط صلاحيات دور الحارس (تسجيل الحضور)
- [x] ربط صلاحيات دور المشرف (إدارة العمال والحضور)
- [x] ربط صلاحيات دور المحاسب (التقارير المالية)
- [x] ربط صلاحيات دور الشؤون الإدارية (المستخدمين ومراكز التكلفة)
- [x] ربط صلاحيات دور المدير المالي (الموافقة على الرواتب)
- [x] ربط صلاحيات دور المدير العام (جميع الصلاحيات)
- [x] اختبار الصلاحيات في صفحة "صلاحيات الأدوار"


## تحسين صفحة إدارة الصلاحيات
- [x] إخفاء عمود "كود الصلاحية" من الجدول
- [x] إعادة ترتيب العناوين والأعمدة
- [x] تحسين التنسيق والمظهر العام
- [x] اختبار الصفحة


## تحسين form إضافة صلاحية
- [x] نقل حقل "الفئة" ليكون في الأعلى
- [x] إنشاء كود الصلاحية تلقائياً عند اختيار الفئة وكتابة الاسم
- [x] عرض كود الصلاحية المقترح
- [x] اختبار إضافة صلاحية جديدة


## إضافة أوصاف تفصيلية للصلاحيات
- [x] جلب جميع الصلاحيات من قاعدة البيانات
- [x] كتابة أوصاف تفصيلية لكل صلاحية
- [x] تحديث الأوصاف في قاعدة البيانات
- [x] اختبار عرض الأوصاف في صفحة إدارة الصلاحيات


## إضافة نظام البحث والتصفية في صفحة إدارة الصلاحيات
- [x] إضافة حقل بحث مع أيقونة
- [x] إضافة زر X لمسح البحث
- [x] إضافة أزرار تصفية حسب الفئة (6 أزرار)
- [x] إضافة عداد النتائج المطابقة
- [x] إضافة زر "مسح الفلاتر"
- [x] اختبار البحث بكلمة "عرض" (9 نتائج)
- [x] اختبار التصفية بفئة "المالية والرواتب" (5 نتائج)
- [x] اختبار زر مسح الفلاتر


## تطبيق التحكم بالوصول في Backend (Permission Middleware)
- [x] تحليل الهيكل الحالي لـ routers.ts
- [x] تحليل جدول الصلاحيات والأدوار
- [x] إنشاء دالة مساعدة hasPermission للتحقق من صلاحية المستخدم
- [x] إنشاء middleware requirePermission لحماية APIs
- [x] إنشاء دالة getUserWithPermissions لتحميل صلاحيات المستخدم
- [x] تحديث authenticateRequest لتحميل الصلاحيات تلقائياً
- [x] تطبيق فحص الصلاحيات على APIs العمال (إضافة، تعديل، حذف)
- [x] تطبيق فحص الصلاحيات على APIs المجموعات (إضافة، تعديل، حذف)
- [x] تطبيق فحص الصلاحيات على APIs الحضور (تسجيل، تعديل)
- [x] تطبيق فحص الصلاحيات على APIs الرواتب (إنشاء، تعديل، اعتماد)
- [x] تطبيق فحص الصلاحيات على APIs المستخدمين (إضافة، تعديل، حذف)
- [x] تطبيق فحص الصلاحيات على APIs الأدوار (إضافة، تعديل، حذف)
- [x] تطبيق فحص الصلاحيات على APIs الصلاحيات (إضافة، تعديل)
- [x] اختبار رفض الوصول عند عدم وجود صلاحية
- [x] اختبار السماح بالوصول عند وجود صلاحية
- [x] كتابة vitest tests للتحقق من الصلاحيات (10 tests passed)


## تفعيل إدخال الرمز اليدوي في صفحة الحضور والانصراف
- [x] فحص صفحة AttendanceScanner الحالية
- [x] تحديد المشكلة في خاصية إدخال الرمز اليدوي (الزر معطل disabled)
- [x] تفعيل حقل إدخال الرمز اليدوي
- [x] تحسين واجهة المستخدم للإدخال اليدوي (أيقونة بطاقة، حقل كبير، زر تأكيد)
- [x] إضافة API endpoint getWorkerByCode في Backend
- [x] إضافة دالة handleManualEntry في Frontend
- [x] اختبار الخاصية بإدخال رمز عامل (W001)
- [x] التأكد من عمل API وعرض رسائل الخطأ بشكل صحيح


## تحسين نظام المسح الذكي للحضور والانصراف
- [ ] فحص النظام الحالي لمسح QR والإدخال اليدوي
- [ ] التأكد من عرض بيانات العامل بعد المسح (الاسم، الصورة، المجموعة، الوظيفة)
- [ ] تحسين نافذة التأكيد لعرض معلومات واضحة
- [ ] التأكد من عمل زر التأكيد لتسجيل الحضور/الانصراف
- [ ] إنشاء عامل تجريبي مع QR Code للاختبار
- [ ] اختبار مسح QR Code والتأكد من ظهور البيانات
- [ ] اختبار الإدخال اليدوي والتأكد من نفس النتيجة
- [ ] اختبار تسجيل الحضور والانصراف بنجاح

## إخفاء عناصر القائمة غير المصرح بها
- [ ] فحص DashboardLayout الحالي
- [ ] إضافة فحص الصلاحيات لكل رابط في القائمة
- [ ] إخفاء الروابط التي لا يملك المستخدم صلاحية الوصول إليها
- [ ] اختبار القائمة مع مستخدم بصلاحيات محدودة
- [ ] اختبار القائمة مع مستخدم admin


## تحسين نظام المسح الذكي للحضور والانصراف
- [x] فحص نظام المسح الحالي ونافذة التأكيد (موجود وشامل)
- [x] التأكد من عرض بيانات العامل بعد المسح (يعمل بشكل صحيح)
- [x] التأكد من عمل الإدخال اليدوي بنفس الآلية (يعمل بشكل صحيح)
- [x] إصلاح workers.getByCode لاستدعاء getWorkerByManualCode
- [ ] ملاحظة: الإدخال اليدوي يحتاج manual_code في قاعدة البيانات (يبحث أولاً في manual_code ثم code)

## إخفاء عناصر القائمة غير المصرح بها
- [x] فحص DashboardLayout الحالي (موجود بالفعل!)
- [x] إضافة فحص الصلاحيات لكل عنصر في القائمة (موجود)
- [x] إخفاء العناصر غير المصرح بها (موجود)
- [x] إخفاء الأقسام الفارغة تماماً (موجود)
- [x] إصلاح خطأ أمني: إزالة "عرض جميع القوائم إذا لم يكن لديه صلاحيات"


## إصلاح مشكلة اختفاء لوحة التحكم للمستخدم RADMAN
- [x] فحص صلاحيات المستخدم RADMAN في قاعدة البيانات (44 صلاحية موجودة)
...
## اختبار QR Code Scanner الكامل
- [ ] تسجيل الدخول بحساب RADMAN
- [ ] إضافة عامل جديد للاختبار
- [ ] توليد QR Code للعامل الجديد
- [ ] فحص مشكلة QR Code Scanner (الكاميرا)
- [ ] إصلاح أي مشاكل في Scanner
