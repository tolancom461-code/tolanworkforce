...
export const temporaryAssignments = mysqlTable("temporary_assignments", {
  ...
  fromGroupId: int("from_group_id").references(() => groups.id, { onDelete: 'set null', onUpdate: 'cascade' }),
  toCostCenterId: int("to_cost_center_id").notNull().references(() => costCenters.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
  toGroupId: int("to_group_id").notNull().references(() => groups.id, { onDelete: 'cascade', onUpdate: 'cascade' }),
